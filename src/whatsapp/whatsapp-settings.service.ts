...
export class WhatsappSettingsService {
  ...
  buildLocalContextBlock(ctx: WhatsappLocalContext): string {
    ...
    if (ctx.restaurantNeighborhood) lines.push(`Barrio: ${ctx.restaurantNeighborhood}`);
    if (ctx.restaurantCity) lines.push(`Ciudad: ${ctx.restaurantCity}`);
    if (ctx.mapsUrl) lines.push(`Google Maps / ubicación: ${ctx.mapsUrl}`);
    if (ctx.publicPhone) lines.push(`Teléfono del local: ${ctx.publicPhone}`);
    if (ctx.websiteUrl) lines.push(`Sitio web: ${ctx.websiteUrl}`);
    if (ctx.menuUrl) lines.push(`Menú online: ${ctx.menuUrl}`);
    if (ctx.instagramUrl) lines.push(`Instagram: ${ctx.instagramUrl}`);
    ...
    return (
      `CONTEXTO DEL LOCAL (usa esto si preguntan dónde quedan, cómo llegar, tiempos, pagos, etc.; no inventes):\n` +
      lines.map((l) => `- ${l}`).join('\n')
    );
  }
