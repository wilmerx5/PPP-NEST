...
export class WhatsappCatalogService {
  ...
  formatProductChoicePrompt(
    ...
    if (deduped.length === 1) {
      return (
        (opts?.intro || `Encontré *${deduped[0].name}* (cód. ${deduped[0].code}).`) +
    ...
    const body = deduped
      .map((p, i) => {
        ...
        if (label !== p.name) line += `\n   _${p.name}_`;
        if (p.hasAttributes) line += `\n   ↳ Elige opciones al pedirlo`;
        return line;
      })
      .join('\n\n');
      ...
      .map((p) => {
        ...
        if (tokens.length) {
          for (const t of tokens) {
            ...
            else if (nameTokens.some((nt) => fuzzyTokenMatch(t, nt))) score += 16;
  ...
  private optionNumberEmoji(index: number): string {
