export const POINTS_REQUIRED_FOR_PRIZE = 9;

/** Códigos de producto que generan puntos (reglas del negocio). */
export const POINTS_PRODUCT_CODES = {
...
export function buildPointsHelpUrl(websiteUrl?: string | null): string | null {
  const base = (websiteUrl || '').trim().replace(/\/+$/, '');
  if (!base) return null;
  return `${base}/ayuda/puntos`;
}

export function buildPointsOverviewReply(ctx: PointsHelpContext): string {
  const helpUrl = buildPointsHelpUrl(ctx.websiteUrl);
  const balance =
    ctx.availablePoints != null
      ? `\n\n📊 *Tu saldo:* ${ctx.availablePoints} punto(s) disponible(s). ` +
        `Con ${POINTS_REQUIRED_FOR_PRIZE} puedes generar un premio (medio pollo gratis).`
      : '';
  ...
  return (
    ...
    `*¿Qué es?* Cada compra elegible suma puntos. Con *${POINTS_REQUIRED_FOR_PRIZE} puntos* generas un *premio*: ` +
    ...
    `*¿Qué platos suman?*\n` +
    `• Códigos ${POINTS_PRODUCT_CODES.individual.join(', ')}: *1 punto* cada uno.\n` +
    `• Códigos 2 y 5 *juntos* en la misma compra: *1 punto* (medio pollo).\n\n` +
    `*¿Cómo usar un premio?*\n` +
    `1. Junta ${POINTS_REQUIRED_FOR_PRIZE} puntos → *redimir* (genera un código de premio, válido 30 días).\n` +
    ...
    `• *redimir* — cambiar ${POINTS_REQUIRED_FOR_PRIZE} puntos por premio\n` +
...
export function buildRegisterPointSteps(ctx: PointsHelpContext): string {
  const helpUrl = buildPointsHelpUrl(ctx.websiteUrl);
  if (ctx.linkedUserName) {
    return (
      `Para *registrar un punto* de factura:\n\n` +
  ...
  return (
    `Para registrar puntos de factura necesitas *cuenta en la web* con el mismo celular de este WhatsApp.\n\n` +
    `Pasos:\n` +
...
export function buildRedeemSteps(available: number): string {
  if (available < POINTS_REQUIRED_FOR_PRIZE) {
    const missing = POINTS_REQUIRED_FOR_PRIZE - available;
    return (
      `Tienes *${available}* punto(s) disponible(s). ` +
      `Necesitas *${POINTS_REQUIRED_FOR_PRIZE}* para redimir.\n\n` +
      `Te faltan *${missing}*. Sigue acumulando con compras elegibles o registrando códigos de factura.`
    );
  }
  return (
    `Tienes *${available}* puntos ✅\n\n` +
    `Escribe *redimir* para generar tu premio (gasta ${POINTS_REQUIRED_FOR_PRIZE} puntos). ` +
...
export function formatPremioAppliedNote(code: string, expiresAt?: Date | null): string {
  const exp = expiresAt
    ? `\nVence: ${expiresAt.toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' })}`
    : '';
  return (
    `🎁 *Premio anotado:* \`${code}\`${exp}\n` +
    `Se aplicará al confirmar si tu carrito lleva *medio pollo* (cód. 2 o 5).`
  );
}

export function formatCartNeedsHalfChickenForPremio(): string {
  return (
    `Tu premio está anotado, pero el carrito aún no lleva *medio pollo* (producto código *2* o *5*).\n\n` +
