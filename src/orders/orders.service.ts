...
export class OrdersService {
  ...
  /**
   * Lista facturas electrónicas emitidas (o intentadas) en un rango de fechas Bogotá.
   * Filtra por electronic_invoice_status != 'none'.
   */
  async findElectronicInvoices(opts: {
    ...
    page?: number;
    limit?: number;
    /** Permite hasta 10 000 filas (export CSV). */
    exportMode?: boolean;
  }): Promise<{
    from: string;
    to: string;
    ...
    const summaryRows: Array<{ status: string; c: string }> = await this.orderRepo
      .createQueryBuilder('o')
    ...
    const orders = await baseQb()
      .orderBy(
        'COALESCE(o.electronic_invoice_issued_at, o.created_at)',
        'DESC',
      )
      .skip((page - 1) * limit)
      .take(limit)
  ...
  /** CSV para contador (mismo filtro que el listado; máx. 10 000 filas). */
