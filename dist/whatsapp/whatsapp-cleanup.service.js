    ...
    async runSessionExpiry() {
        ...
        try {
            ...
            for (const conv of agentIdle) {
                ...
                if (notify) {
                    await this.safeNotify(conv, 'El equipo no pudo seguir ahora. Vuelve el asistente 🤖 — dime cómo te ayudo o escribe *humano*.');
                }
            }
            ...
            for (const conv of clientIdle) {
                ...
                if (notify) {
                    await this.safeNotify(fresh, 'Como no hubo respuesta, cerramos esta atención por ahora. Cuando quieras pedir, escríbenos de nuevo 👍');
                }
            }
