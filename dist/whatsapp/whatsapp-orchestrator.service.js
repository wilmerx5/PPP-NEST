...
let WhatsappOrchestratorService = WhatsappOrchestratorService_1 = class WhatsappOrchestratorService {
    ...
    async handleIncoming(msg) {
        ...
        if (embeddedProduct && !session.pendingMatch && !session.pendingAttribute) {
            ...
            await this.conversationService.saveSession(conv, session, 'building_cart');
            await this.reply(conv, msg.waId, this.buildCartAddReply(session, cfg.defaultDeliveryFee, embeddedProduct.name, {
                extraLine: [
        ...
        if (resolvedMatches.length === 1 && !session.pendingMatch && !session.pendingAttribute) {
            ...
            session = added.session;
            await this.conversationService.saveSession(conv, session, 'building_cart');
            await this.reply(conv, msg.waId, this.buildCartAddReply(session, cfg.defaultDeliveryFee, one.name, {
                extraLine: [
    ...
    buildSessionSummary(conv, session, deliveryFee) {
        ...
        if (session.pendingAttribute) {
            ...
            const remaining = this.catalogService.getRemainingAttributes(productForRemaining, pa.selected || []);
    ...
    async tryHandleVariantFamily(conv, waId, session, text, products, cfg) {
        ...
        if (picked) {
            ...
            session = added.session;
            await this.conversationService.saveSession(conv, session, 'building_cart');
            await this.reply(conv, waId, this.buildCartAddReply(session, cfg.defaultDeliveryFee, picked.name));
            return true;
        }
        session = {
            ...session,
    ...
    async answerSideQuestionWithAi(params) {
        ...
        const remainingAttrs = this.catalogService.getRemainingAttributes(productForRemaining, pa.selected || []);
    ...
    async tryConfirmOrder(conv, waId, session) {
        ...
        if (session.pendingRedemptionCode &&
            !this.pointsHandler.cartHasHalfChicken(session.cart)) {
            await this.reply(conv, waId, `${(0, whatsapp_points_help_1.formatCartNeedsHalfChickenForPremio)()}\n\nAgrega medio pollo (cód. 2 o 5) y vuelve a *confirmar*.`);
            return;
        }
        try {
    ...
    async tryHandleCartModification(conv, waId, session, text, products, cfg) {
        ...
        if (/^ya\s+no[\s!.?]*$/i.test(trimmed) && session.pendingAttribute) {
            ...
            await this.conversationService.saveSession(conv, session, 'building_cart');
            await this.reply(conv, waId, 'Listo, no agregamos ese producto 👍 ¿Qué más te gustaría?');
            return true;
        }
        if (this.isClearCartIntent(text)) {
            if (!session.cart.length &&
    ...
    async handleProductWithVariants(conv, waId, session, product, text, cfg) {
        ...
        if (step.status === 'partial') {
            session = {
                ...session,
                ...(deliveryHint ? { orderType: 'delivery', address: deliveryHint } : {}),
                pendingAttribute: {
                    productId: product.id,
                    name: product.name,
                    code: product.code,
