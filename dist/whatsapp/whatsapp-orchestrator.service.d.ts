import { WhatsappSettingsService } from './whatsapp-settings.service';
import { WhatsappMetaService, IncomingWhatsappMessage } from './whatsapp-meta.service';
import { WhatsappCatalogService } from './whatsapp-catalog.service';
import { WhatsappAiService } from './whatsapp-ai.service';
import { WhatsappConversationService } from './whatsapp-conversation.service';
import { BusinessService } from '../business/business.service';
import { OrdersService } from '../orders/orders.service';
import { PaymentsService } from '../payments/payments.service';
import { WhatsappActionGuardService } from './whatsapp-action-guard.service';
import { WhatsappPointsService } from './whatsapp-points.service';
import { WhatsappDeliveryRoutingService } from './whatsapp-delivery-routing.service';
import { type OutboundMediaKind } from './whatsapp-outbound-media';
export declare class WhatsappOrchestratorService {
    private readonly settingsService;
    private readonly metaService;
    private readonly catalogService;
    private readonly aiService;
    private readonly conversationService;
    private readonly businessService;
    private readonly ordersService;
    private readonly paymentsService;
    private readonly actionGuard;
    private readonly pointsHandler;
    private readonly deliveryRouting;
    private readonly logger;
    private readonly inboundByWaId;
    private readonly inboundCoalesceByWaId;
    private readonly outboundHoldByWaId;
    private readonly outboundCarryByWaId;
    constructor(settingsService: WhatsappSettingsService, metaService: WhatsappMetaService, catalogService: WhatsappCatalogService, aiService: WhatsappAiService, conversationService: WhatsappConversationService, businessService: BusinessService, ordersService: OrdersService, paymentsService: PaymentsService, actionGuard: WhatsappActionGuardService, pointsHandler: WhatsappPointsService, deliveryRouting: WhatsappDeliveryRoutingService);
    handleIncoming(msg: IncomingWhatsappMessage): Promise<void>;
    private scheduleInboundCoalesceFlush;
    private hasInboundCoalescePending;
    private beginOutboundHold;
    private flushOrDiscardOutboundHold;
    private sendReplyNow;
    private flushInboundCoalesce;
    private enqueueInboundUnlocked;
    private runInboundUnlockedWithOutboundHold;
    private handleIncomingUnlocked;
    private applyActions;
    private toCartLimitsConfig;
    private deliveryFeeFor;
    private hasResolvedDeliveryFee;
    private ensureDeliveryFeeQuoted;
    private recalculateDeliveryFee;
    private tryAddProductToCart;
    private isOnlyThisProductCorrection;
    private buildPendingAttributeSession;
    private resolveOrderQuantity;
    private resolveAddItemQuantity;
    private quantityForMultiSegment;
    private rawOrderSegmentForQuantity;
    private resolveAddQuantity;
    private rememberQuantityHint;
    private clearQuantityHint;
    private handleCartLimitBlocked;
    private cartLineKey;
    private consolidateCart;
    private addProductToCart;
    private toPendingAttribute;
    private attributeFlowOpts;
    private buildSessionSummary;
    private replyLooksLikeProductDump;
    private formatRestaurantLocationReply;
    private tryHandleCategoryBrowse;
    private isPendingListRepromptText;
    private looksLikeFreshOrderIntent;
    private messageRelatesToPendingMatch;
    private shouldAbandonPendingMultiOrder;
    private tryAbandonStalePendingState;
    private tryHandleVariantFamily;
    private tryResolvePendingMatchPick;
    private looksLikeSideQuestion;
    private answerSideQuestionWithAi;
    private formatContinueShoppingPrompt;
    private formatCartTiny;
    private withPreface;
    private formatCartOnly;
    private buildCartAddReply;
    private formatOrderSummary;
    private isReadyToConfirm;
    private formatWaPhoneDisplay;
    private looksLikePhoneNumber;
    private isPhoneOnlyCustomerMessage;
    private normalizeContactPhone;
    private buildAskNameMessage;
    private buildAskFulfillmentMessage;
    private buildAskAddressMessage;
    private buildAskPhoneMessage;
    private tryResolvePhoneConfirmation;
    private tryHandleCheckoutSideAdd;
    private tryConfirmOrder;
    completeAfterMercadoPagoPayment(params: {
        conversationId: number;
        waId: string;
        orderId: number;
    }): Promise<void>;
    sendHumanReply(conversationId: number, body: string, agent: {
        id: string;
        fullName: string;
    }): Promise<void>;
    sendHumanMedia(conversationId: number, file: {
        buffer: Buffer;
        mimetype: string;
        originalname: string;
        size: number;
    }, agent: {
        id: string;
        fullName: string;
    }, caption?: string | null): Promise<{
        success: boolean;
        messageType: OutboundMediaKind;
        mediaId: string;
    }>;
    releaseToBot(conversationId: number, opts?: {
        reason?: 'manual' | 'agent_idle';
        notify?: boolean;
    }): Promise<{
        released: boolean;
    }>;
    private shortQuote;
    private resolveAudioToText;
    private static readonly HUMAN_CONTACT_PHONE;
    private humanContactMessage;
    private humanHelpHint;
    private handlePostOrderFollowUp;
    private getDeliveryEtaRangeText;
    private formatDeliveryEtaSentence;
    private tryResolvePendingOrderStatusLookup;
    private tryHandleDeliveryEtaInquiry;
    private replyOrderProgressOrAskNumber;
    private tryHandleInterruptedPhoneOrderInquiry;
    private isProductCompositionQuestion;
    private findProductsForCompositionQuestion;
    private isCompositionFollowUpWithoutProductName;
    private findProductForCompositionQuestion;
    private buildProductCompositionReply;
    private rememberProductFocus;
    private resolveDiscussedProduct;
    private removeCartLinesForProductId;
    private removeCartLinesForVariantFamily;
    private tryHandleComboAvailabilityQuestion;
    private tryHandleVariantPreferenceChange;
    private tryHandleLargerPackInquiry;
    private tryHandleServingSizeChange;
    private tryApplyVariantPreferenceToProduct;
    private tryHandleProductCompositionQuestion;
    private tryResolvePendingCompositionAsk;
    private resolveImageOrderText;
    private isVagueImageCaption;
    private resolveImageMessage;
    private isLikelyPaymentProofContext;
    private looksLikeFailedMediaNotice;
    private normalizeForMatch;
    private isClearCartIntent;
    private extractCartRemovalQuery;
    private formatCartLineLabel;
    private matchCartItemsForRemoval;
    private removeCartLines;
    private extractAddressChangeTarget;
    private tryHandleAddressChange;
    private tryAbandonPendingSelection;
    private tryHandleCartItemReplacement;
    private tryHandleCartComplaintCorrection;
    private tryHandleCartModification;
    private isCancelIntent;
    private formatOrderStatusLabel;
    private handleCancelRequest;
    private isConfirmKeyword;
    private confirmTokenMatches;
    private simpleEditDistance;
    private isGreetingKeyword;
    private stripLeadingGreeting;
    private isVagueOrderIntent;
    private buildAskWhatToOrderMessage;
    private isMenuLinkIntent;
    private isPaymentLinkRequest;
    private isPickupIntent;
    private isDeliveryIntent;
    private applyPickupIntent;
    private extractEtaPhrase;
    private withDeliveryAddress;
    private extractDeliveryInstructionNote;
    private applyDeliveryHintFromMessage;
    private resolveCustomerIntent;
    private tryHandleInlineOrderNoteEarly;
    private tryHandleConfirmCurrentAddress;
    private tryHandleAddressClarification;
    private tryHandleAddressOnlyWhileBuildingCart;
    private tryAppendUnitDetailsToAddress;
    looksLikeUnitOrTowerDetailOnly(text: string): boolean;
    looksLikeAddressReplacement(text: string): boolean;
    private normalizeUnitDetailText;
    private tryAppendDeliveryAccessReference;
    looksLikeDeliveryAccessReference(text: string): boolean;
    private isAddressOnlyCustomerMessage;
    private stripDeliveryAddressPreface;
    private truncateAddressAfterContactClauses;
    private normalizeDeliveryAddress;
    private splitAddressCustomerHint;
    private mergeGeocodedAddressWithCustomerHint;
    private streetsClearlyDisagree;
    private isPlausibleDeliveryAddress;
    private looksLikeExplicitLandmarkKeyword;
    private looksLikeLandmarkOrComplexName;
    private isStrongExplicitAddress;
    private looksLikeFoodNotAddress;
    private extractDeliveryTail;
    private isPickupOnlyDeliveryClause;
    private splitProductAndDelivery;
    private parseCompoundOrderMessage;
    private mergeNameScores;
    private looksLikeAddress;
    private looksLikeAddressRejectingPersonName;
    private formatLocationAddress;
    private formatDeliverySetupEmptyCartReply;
    private buildAskNotesMessage;
    private looksLikeStandaloneOrderNote;
    private extractCartItemNotePayload;
    private resolveCartNoteTargetIndex;
    private applyInlineOrderNote;
    private readonly CASH_CHANGE_AMOUNT;
    private extractCashChangeFromText;
    private stripCashChangePhrases;
    private formatInlineNoteAck;
    private appendCustomerNote;
    private applyNotesFromText;
    private looksLikePayment;
    private looksLikePaymentMethodQuestion;
    private tryExplainPaymentMethodOption;
    private resolvePaymentChoice;
    private buildPaymentConfirmReply;
    private buildWelcomeMessage;
    private buildAiDisclaimerMessage;
    private replyFirstContactWelcome;
    private formatOrderSuccessMessage;
    private resolveOrderThanksMessage;
    private isMultiOrderAffirmative;
    private handleProductWithVariants;
    private tryHandleMixtoCompositionInquiry;
    private tryHandleComboExplanation;
    private splitPriceAndOrderParts;
    private tryHandleProductInfoInquiry;
    private formatPriceInquiryReply;
    private savePendingAddOffer;
    private tryHandlePendingAddOffer;
    private tryHandleUpcomingAddressIntent;
    private tryHandlePaymentCapabilityQuestion;
    private toPendingMultiProduct;
    private formatMultiOrderProposal;
    private sessionFromMultiResolve;
    private addPendingMultiConfidentToCart;
    private tryPreferChickenComboForFoodDrink;
    private tryHandleMultiProductOrder;
    private tryResolvePendingMultiOrder;
    private popCompletedNeedsAttribute;
    private findNewProductOrderCandidate;
    private tryAddProductDuringPendingAttribute;
    private tryHandleDeliverySetup;
    private tryApplyAiClassify;
    private isBusinessServiceInquiry;
    private tryHandleBusinessServiceInquiry;
    private tryHandleCoverageInquiry;
    private tryHandlePointsFlow;
    private reply;
}
