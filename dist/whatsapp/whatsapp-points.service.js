...
let WhatsappPointsService = class WhatsappPointsService {
    ...
    isAwaitingPointCodePrompt(text) {
        const t = (text || '').trim();
        if (this.extractPointCodeCandidate(t))
            return false;
        return /\b(mira|aqu[ií]|te\s+paso|ah[ií]\s+va|te\s+envio|te\s+env[ií]o)\s+(el\s+)?c[oó]digo\b/i.test(t);
    }
