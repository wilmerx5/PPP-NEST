...
let WhatsappCatalogService = class WhatsappCatalogService {
    ...
    findFoodDrinkCompanionProduct(text, known, products) {
        ...
        if (!drinkMatch)
            return null;
        const scored = this.searchByNameScored(drinkMatch[0], products, 6).filter((x) => this.isLikelyDrinkProduct(x.p));
        if (scored.length && scored[0].score >= 35)
            return scored[0].p;
        return null;
    }
    ...
    resolveMultiProductOrder(text, products) {
        ...
        for (const rawSegment of segments) {
            ...
            if (this.looksLikeFoodPlusDrinkOrder(text) &&
                ...
                if (drinks.length >= 1) {
                    const personal = drinks.find((x) => /\bpersonal\b/.test(normalizeText(x.p.name))) || drinks[0];
