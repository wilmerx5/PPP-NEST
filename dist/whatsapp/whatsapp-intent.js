...
function looksLikeClearCartMessage(text) {
    ...
    if (!t)
        return false;
    if (/^(reiniciar|reinicia|reinciar|empezar\s+de\s+nuevo|empezar\s+otra\s+vez|nuevo\s+pedido|otro\s+pedido|borrar\s+carrito|limpiar\s+carrito|vaciar\s+carrito|vaciar\s+pedido|borrar\s+pedido|borrar\s+todo|quitar\s+todo|limpiar\s+todo|vaciar\s+todo|limpiar|limpia|vaciar|vacia)$/.test(t)) {
        return true;
    }
    const clearVerb = '(?:limpiar|limpia|vaciar|vacia|vacio|borrar|borra|quitar|quita|eliminar|elimina|sacar|saca|resetear|resetea|reiniciar|reinicia|reinciar)';
...
const HUMAN_RE = /\b(humano|asesor|persona|operador|alguien\s+del\s+(?:local|restaurante)|hablar\s+con\s+(?:alguien|una\s+persona))\b/i;
const PAYMENT_RE = /\b(contraentrega|efectivo|cash|transferencia|nequi|daviplata|mercadopago|mercado\s*pago|pago\s+con)\b/i;
...
const STREET_ADDRESS_RE = /\b(calle|carrera|cra|cll|av\.?|avenida|diag(?:onal)?|transversal|barrio|habitaci[oó]n|apto|apartamento|torre)\b/i;
function looksLikeNonAddressCommand(text) {
    ...
    if (/\b(cancelar|cancela|anular|anula)\b/.test(t))
        return true;
    if (/\b(humano|asesor|persona|agente)\b/.test(t))
        return true;
...
function looksLikeAddressOnlyMessage(text, hints = {}) {
    ...
    if (looksLikeNonAddressCommand(raw))
        return false;
    if (/\b(pollo|sopa|bandeja|mojarra|churrasco|hamburguesa|ajiaco|mondongo|gaseosa|limonada|broaster|arepa|combo|ejecutivo)\b/i.test(raw)) {
        return false;
    }
...
function classifyWhatsappCustomerIntent(hints) {
    ...
    if (!text)
        return 'other';
    if (hints.isHumanRequest || HUMAN_RE.test(text))
        return 'human';
    if (looksLikeClearCartMessage(text))
        return 'clear_cart';
