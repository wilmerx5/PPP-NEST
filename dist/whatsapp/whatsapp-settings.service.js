...
const DEFAULT_WELCOME = '¡Hola! 👋 *{brand}*. ¿Qué se te antoja?\nMenú: {menuUrl}';
const DEFAULT_AI_DISCLAIMER = whatsapp_human_contact_1.WHATSAPP_AI_DISCLAIMER_SAFE;
const DEFAULT_MENU_LINK = 'Menú 👇\n{menuUrl}\n\nDime qué quieres (nombre o código).';
const DEFAULT_HUMAN_HANDOFF = whatsapp_human_contact_1.WHATSAPP_HUMAN_CONTACT_MESSAGE;
const DEFAULT_ORDER_SUCCESS = 'Gracias por pedirnos 🍗 Enseguida lo preparamos.';
const DEFAULT_CLOSED_MESSAGE = 'Estamos *cerrados*. Cuando abramos escríbenos de nuevo.';
const DEFAULT_LARGE_ORDER_HANDOFF = `Ese pedido es grande para WhatsApp.\n${whatsapp_human_contact_1.WHATSAPP_HUMAN_CONTACT_MESSAGE}`;
const TONE_GUIDE = `
...
let WhatsappSettingsService = class WhatsappSettingsService {
    ...
    async getEffectiveConfig() {
        ...
        return {
            ...
            largeOrderHandoffMessage: (0, whatsapp_human_contact_1.scrubAsesorHandoffCopy)(this.applyTemplate(largeOrderTpl, templateVars), DEFAULT_LARGE_ORDER_HANDOFF),
            ...
            aiDisclaimerMessage: (0, whatsapp_human_contact_1.scrubAiDisclaimerCopy)(this.applyTemplate(aiDisclaimerTpl, templateVars)),
            menuLinkMessage: this.applyTemplate(menuLinkTpl, templateVars),
            humanHandoffMessage: (0, whatsapp_human_contact_1.scrubAsesorHandoffCopy)(this.applyTemplate(humanTpl, templateVars)),
    ...
    buildLocalContextBlock(ctx, menuConceptGroups) {
        ...
        lines.push(`Tiempo de domicilio (orientativo): ${ctx.deliveryTimeNote?.trim() || 'unos 35–45 minutos'}`);
