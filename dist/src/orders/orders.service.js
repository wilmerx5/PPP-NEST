...
let OrdersService = class OrdersService {
    ...
    constructor(orderRepo, itemRepo, attrRepo, extraRepo, productRepo, userRepo, gateway, dataSource, pointsService, productsService, businessService, webDelivery, mailService, circuitBreaker, factusService) {
    ...
    assertOrderContentEditable(order) {
        const status = order.electronicInvoiceStatus;
        if (status === 'accepted') {
            throw new common_1.BadRequestException(`Esta orden tiene factura electrónica${order.electronicInvoiceNumber ? ` ${order.electronicInvoiceNumber}` : ''} aceptada. No se puede modificar. Anula la FE (nota crédito) o cancela la orden.`);
        }
        if (status === 'pending') {
            throw new common_1.BadRequestException('Hay una factura electrónica en proceso. Espera el resultado o anúlala antes de modificar la orden.');
        }
    }
    async cancelOrderFully(orderId, options) {
        ...
        const skipNcOnOrderCancel = (process.env.FACTUS_SKIP_NC_ON_ORDER_CANCEL ?? 'false').toLowerCase() ===
            'true';
        let creditNoteNumber = null;
        if (order.electronicInvoiceStatus === 'accepted' &&
            order.electronicInvoiceNumber) {
            if (skipNcOnOrderCancel) {
                this.logger.warn(`[cancel] TEMP skip NC — orden=#${orderId} FE ${order.electronicInvoiceNumber} ` +
                    `queda en Factus; se cancela solo la orden PPP`);
            }
            else {
                this.logger.log(`[cancel] orden=#${orderId} tiene FE ${order.electronicInvoiceNumber} — emitiendo NC`);
                try {
                    ...
                    if (!nc?.success) {
                        throw new common_1.BadRequestException(nc?.message ||
                    ...
                    if (err instanceof common_1.ConflictException) {
                        this.logger.warn(`[cancel] orden=#${orderId} FE ya tenía NC; se continúa cancelando la orden`);
                    }
                        ...
                        throw new common_1.BadRequestException(`No se pudo anular la factura electrónica (${order.electronicInvoiceNumber}): ${message}. La orden NO se canceló.`);
                    }
                }
            }
        }
        ...
        return {
            ...
            message: wasCompleted
                ? `Orden #${order.dailyOrderNumber ?? orderId} completada anulada (force). Inventario restaurado.${creditNoteNumber
                    ...
                    : skipNcOnOrderCancel && order.electronicInvoiceNumber
                        ...
                        : ''}`
                : `Orden #${order.dailyOrderNumber ?? orderId} cancelada${creditNoteNumber
                    ? `. FE anulada con NC ${creditNoteNumber}`
                    : skipNcOnOrderCancel && order.electronicInvoiceNumber
                        ? `. FE ${order.electronicInvoiceNumber} se mantiene (sin NC)`
                        : ''}`,
    ...
    async updateOrderGeneral(orderId, dto) {
        ...
        const touchesContent = dto.customerName !== undefined ||
            dto.phone !== undefined ||
    ...
    async findTodayOrdersByPhone(phone) {
        ...
        const rows = await this.orderRepo
            ...
            .andWhere(`REPLACE(REPLACE(REPLACE(REPLACE(o.phone, ' ', ''), '+', ''), '-', ''), ' ', '') LIKE :tail`, { tail: `%${tail}` })
