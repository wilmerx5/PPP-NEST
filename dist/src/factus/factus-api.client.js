...
let FactusApiClient = FactusApiClient_1 = class FactusApiClient {
    ...
    async listBills(params) {
        ...
        if (params?.perPage != null)
            q.set('filter[per_page]', String(params.perPage));
        if (params?.referenceCode)
            q.set('filter[reference_code]', params.referenceCode);
        if (params?.status != null)
            q.set('filter[status]', String(params.status));
        const qs = q.toString();
        return this.requestJson('GET', `/v2/bills${qs ? `?${qs}` : ''}`);
    }
